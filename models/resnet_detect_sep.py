...
class ConvBlock3(nn.Module):
	...
	def __init__(self, in_channels, out_channels):
		...
		self.shortcut = conv1x1x1(in_channels, out_channels) if in_channels != out_channels else nn.Identity()
		self.norm_out = nn.GroupNorm(num_groups=min(8, out_channels), num_channels=out_channels, affine=True)
...
class DownConvBlock3(nn.Module):
	...
	def __init__(self, in_channels, out_channels):
		...
		self.pre_conv = ConvBlock3(in_channels, in_channels)
		self.maxpool = maxpool1x2x2()
		self.post_conv = ConvBlock3(in_channels, out_channels)
...
class UpConvBlock3(nn.Module):
	...
	def __init__(self, in_channels, out_channels):
		...
		self.post_conv = ConvBlock3(in_channels//2 + out_channels, out_channels)
